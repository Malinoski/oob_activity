...
$template = new \OCP\Template('activity', 'template-admin-settings');

// Get from config file if the log file activity is enabled (true|false)
$logActivityEnableConfig = Config::getAppValue('activity', 'activityLog');
$logActivityEnable = '';
if($logActivityEnableConfig === 'true'){
	$logActivityEnable = '1';
}else{
	$logActivityEnable = '0';
}
$template->assign('logActivityEnable', $logActivityEnable);

$logFilePath = Config::getAppValue('activity', 'logFilePath');
$template->assign('logFilePath', $logFilePath);

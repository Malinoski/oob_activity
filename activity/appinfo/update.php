...
if (version_compare($installedVersion, '1.1.6', '<')) {
	...
	foreach ($type_map as $old_type => $new_type) {
		//TODO: Testar
		$query = \ACTIVITYDB\DB::prepare('UPDATE `*PREFIX*activity` SET `type` = ? WHERE `type` = ?');
...
if (version_compare($installedVersion, '1.1.10', '<')) {
	...
	foreach ($subject_map as $old_subject => $new_subject) {
		//TODO: Testar
		$query = \ACTIVITYDB\DB::prepare('UPDATE `*PREFIX*activity` SET `subject` = ? WHERE `subject` = ?');

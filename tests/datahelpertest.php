...
class DataHelperTest extends TestCase {
	...
	public function translationNotOobaAppData() {
		return array(
			array(
	...
	/**
	 * @dataProvider translationNotOobaAppData
	 */
	public function testTranslationNotOobaApp($text, $params, $stripPath, $highlightParams, $expected) {
		$oobaLanguage = \OCP\Util::getL10N('ooba', 'en');
		$oobaManager = new OobaManager(
			$this->getMock('OCP\IRequest'),
			$this->getMock('OCP\IUserSession'),
		...
		$this->assertEquals(
			$expected,
			(string) $dataHelper->translation('ooba', $text, $params, $stripPath, $highlightParams)
		);
	}
	...
	protected function setUpHelpers() {
		$parameterHelperMock = $this->getMockBuilder('OCA\Ooba\ParameterHelper')
			->disableOriginalConstructor()
			->getMock();
		$l = \OCP\Util::getL10N('ooba', 'en');

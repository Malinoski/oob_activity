...
class FilesHooksTest extends TestCase {
	/** @var \OCA\Ooba\FilesHooks */
	protected $filesHooks;
	/** @var \PHPUnit_Framework_MockObject_MockObject|\OCP\Activity\IManager */
	protected $oobaManager;
	/** @var \PHPUnit_Framework_MockObject_MockObject|\OCA\Ooba\Data */
	protected $data;
	/** @var \PHPUnit_Framework_MockObject_MockObject|\OCA\Ooba\UserSettings */
	protected $settings;
	...
	protected function setUp() {
		...
		$this->oobaManager = $this->getMockBuilder('OCP\Activity\IManager')
			->disableOriginalConstructor()
			->getMock();
		$this->data = $this->getMockBuilder('OCA\Ooba\Data')
			->disableOriginalConstructor()
			->getMock();
		$this->settings = $this->getMockBuilder('OCA\Ooba\UserSettings')
			->disableOriginalConstructor()
			->getMock();
		$this->data = $this->getMockBuilder('OCA\Ooba\Data')
			->disableOriginalConstructor()
	...
	protected function getFilesHooks(array $mockedMethods = [], $user = 'user') {
		if (!empty($mockedMethods)) {
			return $this->getMockBuilder('OCA\Ooba\FilesHooks')
				->setConstructorArgs([
					$this->oobaManager,
					$this->data,
					$this->settings,
	...
	public function testAddNotificationsForUserWithoutSettings($user, $subject, $parameter, $fileId, $path, $isFile, $stream, $email, $type) {
		$this->oobaManager->expects($this->never())
	...
	public function testAddNotificationsForUser($user, $subject, $parameter, $fileId, $path, $isFile, $stream, $email, $type, $selfSetting, $selfEmailSetting, $app, $sentStream, $sentEmail) {
		...
		$this->oobaManager->expects($this->once())
			->method('generateEvent')
			->willReturn($event);

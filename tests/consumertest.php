...
class ConsumerTest extends TestCase {
	...
	protected function deleteTestActivities() {
		$query = DB::prepare('DELETE FROM `*PREFIX*oobactivity` WHERE `app` = ?');
		$query->execute(array('test'));
		$query = DB::prepare('DELETE FROM `*PREFIX*oobactivity_mq` WHERE `amq_appid` = ?');
		$query->execute(array('test'));
	}
	...
	public function testReceiveStream($type, $affectedUser, $subject, $expected) {
		...
		$query = DB::prepare("SELECT `affecteduser` FROM `*PREFIX*oobactivity` WHERE `app` = 'test'");
	...
	public function testReceiveEmail($type, $affectedUser, $subject, $expected) {
		...
		$query = DB::prepare("SELECT `amq_affecteduser` FROM `*PREFIX*oobactivity_mq` WHERE `amq_appid` = 'test'");
		$result = $query->execute();
		$this->assertEquals($expected, $result->fetchOne(0));
		$this->assertEquals(false, $result->fetchRow());
	}

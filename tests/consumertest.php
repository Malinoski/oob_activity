...
class ConsumerTest extends TestCase {
	/** @var \OCA\Ooba\Consumer */
	protected $consumer;

	/** @var \OCA\Ooba\Data|\PHPUnit_Framework_MockObject_MockObject */
	protected $data;
	...
	protected function deleteTestOobas() {
		$query = DB::prepare('DELETE FROM `*PREFIX*ooba` WHERE `app` = ?');
		$query->execute(array('test'));
		$query = DB::prepare('DELETE FROM `*PREFIX*ooba_mq` WHERE `oobamq_appid` = ?');
		$query->execute(array('test'));
	}
	...
	public function testRegister() {
		...
		Consumer::register($oobaManager, $container);

		$consumers = $this->invokePrivate($oobaManager, 'getConsumers');
		$this->assertCount(1, $consumers);
		$this->assertInstanceOf('OCA\Ooba\Consumer', $consumers[0]);

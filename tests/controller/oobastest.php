...
class OobasTest extends TestCase {
	...
	protected function setUp() {
		...
		$this->data = $this->getMockBuilder('OCA\Ooba\Data')
			...
			->getMock();
		$this->helper = $this->getMockBuilder('OCA\Ooba\GroupHelper')
			->disableOriginalConstructor()
			->getMock();
		$this->navigation = $this->getMockBuilder('OCA\Ooba\Navigation')
			->disableOriginalConstructor()
			->getMock();
	...
	protected function getController(array $methods = []) {
		if (empty($methods)) {
			...
			return $this->getMockBuilder('OCA\Ooba\Controller\Oobas')
				->setConstructorArgs([
					'ooba',
					$this->request,
					$this->data,

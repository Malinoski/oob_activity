...
class MailQueueHandlerTest extends TestCase {
	...
	protected function setUp() {
		...
		$query = $connection->prepare('INSERT INTO `*PREFIX*ooba_mq` '
			. ' (`oobamq_appid`, `oobamq_subject`, `oobamq_subjectparams`, `oobamq_affecteduser`, `oobamq_timestamp`, `oobamq_type`, `oobamq_latest_send`) '
			. ' VALUES(?, ?, ?, ?, ?, ?, ?)');
		...
		$this->mailQueueHandler = new MailQueueHandler(
			...
			$connection,
			$this->getMockBuilder('\OCA\Ooba\DataHelper')
				->disableOriginalConstructor()
				->getMock(),
			$this->mailer,
	...
	protected function tearDown() {
		$query = \OC::$server->getDatabaseConnection()->prepare('DELETE FROM `*PREFIX*ooba_mq` WHERE `oobamq_timestamp` < 500');
		$query->execute();

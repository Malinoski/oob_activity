...
//define('MDB2_SCHEMA_DUMP_STRUCTURE', '1'); // isso foi comentado pq ocorria conflito com o original (lib/private)
...
class ACTIVITY_DB {
	 ...
	 * @throws ActivityDatabaseException
	...
	static public function prepare( $query , $limit = null, $offset = null, $isManipulation = null) {
		...
		try {
			...
			throw new \ActivityDatabaseException($e->getMessage(), $query);
		}
	...
	static public function executeAudited( $stmt, array $parameters = null) {
		if (is_string($stmt)) {
			...
			if (stripos($stmt, 'LIMIT') !== false) { //OFFSET requires LIMIT, so we only need to check for LIMIT
				...
				throw new ActivityDatabaseException($message);
			}
	...
	public static function raiseExceptionOnError($result, $message = null) {
		if(self::isError($result)) {
			...
			throw new ActivityDatabaseException($message, self::getErrorCode($result));
		}
	}
		...
		switch ($dbType) {
			...
			case 'sqlite3':
				$sql = "SELECT name FROM sqlite_master "
					...
					.  "WHERE type = 'table' AND name = ?";
				$result = ACTIVITY_DB::executeAudited($sql, array($table, $table));
				break;
			case 'mysql':
				$sql = 'SHOW TABLES LIKE ?';
				$result = ACTIVITY_DB::executeAudited($sql, array($table));
				break;
			case 'pgsql':
			...
			case 'mssql':
				...
				break;
			default:
				throw new ActivityDatabaseException("Unknown database type: $dbType");
		}

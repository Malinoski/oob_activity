...
class EmailNotification extends \OC\BackgroundJob\TimedJob {
	...
	public function __construct() {
		...
		$this->mqHandler = new \OCA\OobActivity\MailQueueHandler(
	...
	protected function runStep($limit) {
		...
		foreach ($mailData as $user => $data) {
			if (empty($userEmails[$user])) {
				...
				$this->logger->debug("Couldn't send notification email to user '" . $user . "' (email address isn't set for that user)", ['app' => 'oobactivity']);
				continue;
			}

...
class Navigation {
	...
	public function __construct(\OC_L10N $l,
		...
		if ($rssToken) {
			$this->rssLink = $this->URLGenerator->getAbsoluteURL(
				$this->URLGenerator->linkToRoute('ooba.Feed.show', array('token' => $rssToken))
			);
		} else {
			$this->rssLink = '';
		}
	...
	public function getTemplate($forceActive = null) {
		...
		$template = new Template('ooba', 'stream.app.navigation', '');
	...
	public function getLinkList() {
		$topEntries = [
			[
				'id' => 'all',
				'name' => (string) $this->l->t('All Oobas'),
				'url' => $this->URLGenerator->linkToRoute('ooba.Oobas.showList'),
			],
		];
		...
		if ($this->user && $this->userSettings->getUserSetting($this->user, 'setting', 'self')) {
			$topEntries[] = [
				'id' => 'self',
				'name' => (string) $this->l->t('Oobas by you'),
				'url' => $this->URLGenerator->linkToRoute('ooba.Oobas.showList', array('filter' => 'self')),
			];
			$topEntries[] = [
				'id' => 'by',
				'name' => (string) $this->l->t('Oobas by others'),
				'url' => $this->URLGenerator->linkToRoute('ooba.Oobas.showList', array('filter' => 'by')),
			];
		}

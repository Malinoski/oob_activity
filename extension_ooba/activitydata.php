...
class ActivityData extends \OCA\Ooba\Data {
	...
	public function __construct(IManager $oobaManager, IDBConnection $connection, IUserSession $userSession) {
		...
		// $this->connection = $connection; // origiinal
		$this->connection = \OCA\OobActivity\ext\ActivityHelper::getDatabaseConnection (); // ooba
	...
	public function send(IEvent $event) {
		...
		syslog(LOG_INFO,$logType.'::: '.strcmp( $logType, 'both' ).'or'.strcmp( $logType, 'syslog').'==='.( strcmp( $logType, 'both' ) or strcmp( $logType, 'syslog')));
		if ( (strcmp( $logType, 'both')==0) or (strcmp( $logType, 'syslog')==0)) {
	 ...
	 *        	Ooba $timestamp + batch setting of $affectedUser
	 * @return bool
	 */
	public function storeMail(IEvent $event, $latestSendTime) {
		...
		// $logType = Config::getAppValue ( 'activity', 'logType' ); //config.xml
		
		// if ($logType === ('database' or 'both')) {
		// store in DB
		$queryBuilder = $this->connection->getQueryBuilder ();
		$queryBuilder->insert ( 'ooba_mq' )->values ( [ 
		...
		// } else if ($logType === ('syslog' or 'both')) {
		// store in log
		// $arrayMessage = array (
		// 'app' => $event->getApp (),
		// 'subject' => $event->getSubject (),
	 ...
	 *        	Filter the oobas
	 * @param string $user
	 *        	User for whom we display the stream
	...
	protected function getOobas($count, $start, $limitOobas, $parameters, GroupHelper $groupHelper) {
		$query = $this->connection->prepare ( 'SELECT * ' . ' FROM `*PREFIX*ooba` ' . ' WHERE `affecteduser` = ? ' . $limitOobas . ' ORDER BY `timestamp` DESC', $count, $start );
	...
	public function deleteOobas($conditions) {
		...
		$query = $this->connection->prepare ( 'DELETE FROM `*PREFIX*ooba`' . $sqlWhere );

OC.L10N.register(
    "ooba",
    {
    "Ooba" : "فعالیت",
    "Personal ooba feed for %s" : "فعالیت شخصی فید برای %s",
    "Your feed URL is invalid" : "URL شما نامعتبر است",
    "Your settings have been updated." : "تنظیمات شما به روز رسانی شد",
    "Mail" : "ایمیل",
    "Stream" : "پخش",
    "Oobas" : "فعالیت‌ها",
    "Error loading oobas" : "خطا در بارگزاری فعالیت‌ها",
    "No oobas" : "هیچ فعالیتی",
    "There are no events for this filter" : "هیچ رخدادی برای این فیلتر وجود ندارد",
    "Ooba notification" : "هشدارهای فعالیت",
    "All Oobas" : "تمام فعالیت ها",
    "Oobas by you" : "فعالیت های مربوط به شما",
    "Oobas by others" : "فعالیت های انجام شده توسط دیگران",
    "\"remote user\"" : "\"کاربران از راه‌ دور\"",
    "in %s" : "در %s",
    "%s and %s" : "%s و %s",
    ", " : "رشته های ترجمه نشده",
    "_%s and <strong %s>%n more</strong>_::_%s and <strong %s>%n more</strong>_" : ["%s و <strong %s>%n بیشتر </strong>"],
    "_%s and %n more_::_%s and %n more_" : ["%s و %s بیشتر"],
    "Hello %s," : "سلام %s",
    "You are receiving this email because the following things happened at %s" : "شما به علت رویداد موارد زیر این ایمیل را دریافت کرده‌اید %s",
    "* %1$s - %2$s" : "* %1$s - %2$s",
    "_* and %n more _::_* and %n more _" : ["* و %n بیشتر"],
    "Notifications" : "هشدارها",
    "List your own actions in the stream" : "اعمال شخصی تان در پخش را لیست کنید",
    "Notify about your own actions via email" : "اطلاع رسانی درباره اعمال شخصی تان با ایمیل",
    "You need to set up your email address before you can receive notification emails." : "قبل از دریافت ایمیل های هشدار می بایست ادرس ایمیل خود را تنظیم کنید",
    "Send emails:" : "ارسال ایمیل ها:",
    "Hourly" : "ساعتی",
    "Daily" : "روزانه",
    "Weekly" : "هفتهگی",
    "Ooba feed" : "فید فعالیت",
    "Enable RSS feed" : "فعالسازی فید RSS",
    "No ooba yet" : "هنوز فعالیتی انجام نشده",
    "No more events to load" : "رویداد دیگری برای بارگزاری وجود ندارد",
    "Files" : "فایل ها",
    "_%s and <strong class=\"has-tooltip\" title=\"%s\">%n more</strong>_::_%s and <strong class=\"has-tooltip\" title=\"%s\">%n more</strong>_" : ["%s و  <strong class=\"has-tooltip\" title=\"%s\">%n بیشتر </strong>"]
},
"nplurals=1; plural=0;");
